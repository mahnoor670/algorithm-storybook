...
export default function SelectionSort() {
  ...
  const fetchLeaderboard = async () => {
    const res = await fetch(`${API}/leaderboard?game=selection-sort`);
  ...
  const handleClick = async (index) => {
    ...
    const response = await fetch(`${API}/selection-sort/pick`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ arr: [...grades], picked_index: index, sorted_count: sortedCount, steps: steps })
    });
  ...
  return (
    <>
      ...
      <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", fontFamily: "'Ubuntu', sans-serif", overflow: "hidden", position: "relative", background: "transparent" }}>
        ...
        <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", backgroundImage: "url('/exam.jpg')", backgroundSize: "cover", backgroundPosition: "center", filter: "brightness(0.5)", zIndex: 0 }} />
        ...
        <div style={{ textAlign: "center", padding: "24px 20px 8px", zIndex: 10, position: "relative" }}>
          <h1 style={{ color: "#FF5F05", fontSize: "2.5rem", margin: 0, textShadow: "0 2px 8px rgba(0,0,0,0.5)" }}>{won ? "📚 Exams Sorted!" : "Professor's Stack"}</h1>
          <p style={{ color: "#fff", fontSize: "0.95rem", margin: "4px 0 0", textShadow: "0 1px 4px rgba(0,0,0,0.6)" }}>A Selection Sort Adventure</p>
        </div>

        <div style={{ display: "flex", justifyContent: "center", padding: "8px 20px", zIndex: 10, position: "relative" }}>
          <div style={{ background: won ? "rgba(255,220,50,0.96)" : "rgba(255,255,255,0.93)", borderRadius: "16px", padding: "14px 24px", maxWidth: "680px", width: "100%", textAlign: "center", boxShadow: "0 6px 24px rgba(0,0,0,0.25)", transition: "background 0.4f" }}>
        ...
        <div style={{ position: "fixed", bottom: "100px", left: 0, right: 0, display: "flex", justifyContent: "center", gap: "14px", alignItems: "flex-end", zIndex: 10, padding: "0 40px" }}>
          {grades.map((grade, index) => {
            const isSorted = index < sortedCount;
            return (
              <div key={index} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "8px" }}>
                <div style={{ fontSize: "1.8rem" }}>{isSorted ? "✅" : won ? "🎉" : "📄"}</div>
                <div className="exam-btn" onClick={() => handleClick(index)} style={{ width: "70px", height: "90px", background: isSorted ? "linear-gradient(135deg, #2ecc71, #27ae60)" : "linear-gradient(135deg, #5dade2, #2e86c1)", borderRadius: "8px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: "white", fontWeight: "bold", cursor: isSorted || won ? "not-allowed" : "pointer", opacity: isSorted ? 0.7 : 1, boxShadow: isSorted ? "none" : "0 4px 14px rgba(0,0,0,0.35)", transition: "all 0.3s" }}>
                  <span style={{ fontSize: "1.3rem" }}>{grade}</span>
                  <span style={{ fontSize: "0.65rem", marginTop: "4px" }}>pts</span>
                </div>
                <span style={{ color: "white", fontSize: "0.7rem", background: isSorted ? "rgba(46,204,113,0.6)" : "rgba(0,0,0,0.45)", padding: "2px 7px", borderRadius: "8px" }}>{isSorted ? "sorted" : `#${index + 1}`}</span>
              </div>
            );
          })}
        </div>
      ...
      {showLeaderboard && (
        <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, animation: "fadeIn 0.3s ease-out" }}>
          <div style={{ background: "white", borderRadius: "24px", padding: "36px", maxWidth: "460px", width: "90%", textAlign: "center", boxShadow: "0 25px 60px rgba(0,0,0,0.5)" }}>
            <h2 style={{ color: "#FF5F05", marginTop: 0 }}>📚 Exams Sorted!</h2>
        ...
        <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
          <div style={{ background: "white", borderRadius: "24px", padding: "40px", maxWidth: "520px", width: "90%", textAlign: "left", boxShadow: "0 25px 60px rgba(0,0,0,0.5)", maxHeight: "85vh", overflowY: "auto" }}>
            ...
            <div style={{ background: "#fff8f0", borderRadius: "12px", padding: "16px", margin: "16px 0", borderLeft: "4px solid #FF5F05" }}>
              <p style={{ margin: 0, color: "#333", lineHeight: "1.8" }}><strong>In this game:</strong> The professor's exams are all out of order. Each round, find the exam with the lowest grade in the unsorted pile and click it. That exam gets placed next in the sorted section!</p>
            </div>
            <div style={{ background: "#e8f4fd", borderRadius: "12px", padding: "16px", margin: "16px 0" }}>
              <p style={{ margin: 0, color: "#13294B", lineHeight: "1.8" }}>💻<strong> Big O Notation:</strong> Selection sort always runs in <strong>O(n²)</strong> time regardless of input. Unlike bubble sort, it does not have a best case of O(n) because it always scans the entire unsorted section every round.</p>
            </div>
